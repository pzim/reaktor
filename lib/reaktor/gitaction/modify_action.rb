...
module GitAction
  class ModifyAction < Action
    ...
    def updatePuppetfile
      ...
      Notification::Notifier.instance.notification = "r10k deploy module for #{module_name}/#{self.branch_name} in progress..."
      begin
        r10k_deploy_module self.module_name
      rescue => e
        Notification::Notifier.instance.notification = "ABORTING r10k deploy due to: #{e.message}"
      else
        Notification::Notifier.instance.notification = "r10k deploy module for #{module_name}/#{self.branch_name} finished"
      end
    end
    def cleanup
