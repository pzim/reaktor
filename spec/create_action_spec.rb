...
describe Reaktor::GitAction::CreateAction do
  ...
  describe '.update_puppet_file' do
    context 'given a module of "foo-bar" and a branch of "feature"' do
      ...
      it 'calls somethinh with the branchname' do
        ...
        allow(puppetfile).to receive(:update_module_ref).with(
          options[:module_name],
          options[:branch_name]
        ).and_return('puppetfile contents')
        allow(puppetfile).to receive(:git_update_ref_msg).and_return('Stuff')
        allow(puppetfile).to receive(:write_new_puppetfile).and_return(true)

        git_work_dir = instance_double('Reaktor::Git::WorkDir')
        allow(Reaktor::Git::WorkDir).to receive(:new).and_return(git_work_dir)
        allow(git_work_dir).to receive(:push).and_return(true)

        # pfile_contents = @puppetfile.update_module_ref(module_name, branch_name)
        expect(puppetfile).to receive(:update_module_ref).with(
          options[:module_name],
          options[:branch_name]
        ).and_return('puppetfile contents')
        ...
        expect(git_work_dir).to receive(:push).with('feature', 'Stuff')
        # TODO: ???
